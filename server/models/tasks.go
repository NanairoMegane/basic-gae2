...
var TaskWhere = struct {
	...
	TaskID:    whereHelperuint64{field: "`tasks`.`task_id`"},
...
var (
	taskAllColumns            = []string{"task_id", "content", "created_at"}
	taskColumnsWithoutDefault = []string{"content", "created_at"}
	taskColumnsWithDefault    = []string{"task_id"}
...
func FindTask(ctx context.Context, exec boil.ContextExecutor, taskID uint64, selectCols ...string) (*Task, error) {
	taskObj := &Task{}
	...
	query := fmt.Sprintf(
		"select %s from `tasks` where `task_id`=?", sel,
	)
...
func (o *Task) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.TaskID = uint64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == taskMapping["task_id"] {
		goto CacheNoHooks
	}
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, taskID uint64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `tasks` where `task_id`=? limit 1)"
